...
const localTranslations: LocalTranslations = {
  ...
  slotMachineInfo: {
    ...
    ru: "🎰 Тут машина SLOT - с комбинацией из 3х картинок - BAR, LEMON, GRAPE, SEVEN 🔢 Тут всего 64 различных вариации - на любую из них можно назначить множитель, фиксированный приз а также добавить Адрес как супер приз. 🏆 \nНапример, вы можете сделать ДЖЕК-ПОТ - на комбинацию 777 - и дать приз человеку х10 от ставки и приз - Адрес на какой-нибудь товар в городе.\n🎰 Для игры в SLOT - необходимо настроить выигрышные комбинации.\nЕсли не знаешь как настроить слоты казино что бы не быть в убытке то вот тебе простая и понятная формула: если минимальная ставка составляет 100₽, то максимальная сумма всех возможных призов по теории вероятности не должна превышать 100₽ × 64 = 6400₽, так как всего в слоте 64 вариации. Это ваша верхняя граница, чтобы казино оставалось прибыльным. \nТеперь эти 6400₽ нужно грамотно распределить между выигрышными комбинациями. Например, комбинация BAR-BAR-BAR может давать x3 от ставки — это 300₽, LEMON-LEMON-LEMON — фиксированный приз 200₽, GRAPE-GRAPE-GRAPE может давать адрес/товар без денежной выплаты, а SEVEN-SEVEN-SEVEN быть джекпотом с x10 от ставки (то есть 1000₽) и дополнительным призом в виде адреса. Все остальные 60 комбинаций оставляются пустыми — без выигрыша. \nТаким образом, общая сумма возможных выигрышей: 300₽ + 200₽ + товар + 1000₽ = около 1500–2000₽. Это значительно меньше максимальной границы в 6400₽, что делает вашу систему устойчивой. При этом, так как шанс выпадения каждой комбинации — 1 к 64, выигрыш выпадает примерно 3–4 раза из 64, и вы остаетесь в плюсе."
  },
  casinoHistory: {
