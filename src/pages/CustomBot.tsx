    ...
    <TutorialPage title={tNew('customBotPageTitle')}>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="grid grid-cols-2 mb-8 w-full max-w-md mx-auto">
          <TabsTrigger value="main">{tNew('mainCustomization')}</TabsTrigger>
          <TabsTrigger value="language">{tNew('languageOnStart')}</TabsTrigger>
        </TabsList>

        {/* Main Customization Tab Content */}
        <TabsContent value="main" className="space-y-6 max-w-4xl mx-auto">
          <section>
        ...
        {/* Language Selection Tab Content */}
        <TabsContent value="language" className="space-y-6 max-w-4xl mx-auto">
          <section>
            <h2 className="text-2xl font-semibold mb-3">{tNew('languageSelectionGuide')}</h2>
            <p className="text-lg mb-6">{tNew('languageSelectionIntro')}</p>
          </section>

          {/* Language Selection Setup Section */}
          <section className="bg-gray-900/40 rounded-lg p-6 border border-gray-700">
            <h3 className="text-xl font-medium mb-4 border-l-4 border-neonBlue pl-3">
              {tNew('setupLanguageSelection')}
            </h3>
            
            <div className="space-y-4">
              <p>{tNew('languageSelectionSteps')}</p>
              
              <ol className="space-y-3 pl-5 list-decimal">
                <li className="pl-2">{tNew('languageStep1')}</li>
                <li className="pl-2">{tNew('languageStep2')}</li>
                <li className="pl-2">{tNew('languageStep3')}</li>
                <li className="pl-2">{tNew('languageStep4')}</li>
              </ol>
              
              <div className="bg-neonBlue/10 border border-neonBlue/30 rounded-md p-4 my-4">
                <p className="font-medium text-neonBlue">{tNew('tip')}:</p>
                <p>{tNew('languageTip')}</p>
              </div>
              
              {/* Example image */}
              <div className="rounded-lg overflow-hidden border border-gray-700 mt-3">
                <img 
                  src={languageSelectionImages[0].src}
                  alt={languageSelectionImages[0].alt}
                  className="w-full h-80 object-contain bg-gray-800/60" 
                />
                <p className="text-center text-sm text-gray-400 mt-2">{tNew('languageSelectionExample')}</p>
              </div>
            </div>
          </section>
          ...
          <section className="bg-gray-900/40 rounded-lg p-6 border border-gray-700">
            ...
            <div className="space-y-3">
              <ul className="space-y-3 pl-5">
                <li className={cn("flex gap-2")}>
                  <span className="text-neonBlue">•</span>
                  <span>{tNew('languagePractice1')}</span>
                </li>
                <li className={cn("flex gap-2")}>
                  <span className="text-neonBlue">•</span>
                  <span>{tNew('languagePractice2')}</span>
                </li>
                <li className={cn("flex gap-2")}>
                  <span className="text-neonBlue">•</span>
                  <span>{tNew('languagePractice3')}</span>
                </li>
                <li className={cn("flex gap-2")}>
                  <span className="text-neonBlue">•</span>
                  <span>{tNew('languagePractice4')}</span>
                </li>
              </ul>
              
              <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-md p-4 mt-4">
                <p className="font-medium text-yellow-500">{tNew('important')}:</p>
                <p>{tNew('languageImportantNote')}</p>
              </div>
            </div>
          </section>
        </TabsContent>
