...
const translations: Translations = {
  ...
  ru: {
    ...
    // Quick Start page content
    quickStartDescription: 'Это руководство по быстрому старту поможет тебе настроить и сконфигурировать админ-панель всего за 11 шагов. Следуйте этим инструкциям, чтобы быстро запустить свои продажи. \nНа главной странице есть графики - в них собраны статистические данные работы вашего магазина \n В вверхней части меню - есть куча разных разделов - основные - это боты - где создаете ботов,сотрудники - где создаете сотрудников краткое описание какое-то, клиенты - это список ваших клиентов кто когда либо писал боту, \n раздел Маркетинг - тут разные промо акции ивсе такое и раздел СКЛАД - основной раздел где можно настроить города, районы, курьеры, товары и адреса. \n Для того чтобы добавить какой либо объект - чаще всего вам необходимо перейти в нужный раздел - и сверху справа найти кнопку "ДОБАВИТЬ" после этого будет форма добавления. \n Для того чтобы удалить что либо - вам необходимо перейти в нужный раздел - выбрать нужный объект и кликнуть на первую колонку - тогда вы перейдете в настройки этого объекта и внизу слева будет кнопка УДАЛИТЬ После подтверждения объект удалится - восстановление не возможно) \n Так же на странице списка объектов - например адресов - есть строка массовых действий.  \n Вы можете выбрать нескобько объектов галочками слева от строчки - и выбрать необходимое действие.  \n Так же на некоторых страницах списка есть у каждой строчки справа разные кнопки действий , они чаще всего располагаются в самой крайней правой колонке. ',
    ...
    quickStartPoint4Note1: 'Данный пункт является не обязательным но необходим в том случае если в вашем магазине существует достаточно много товаров с разной фасовкой например: Группа товаров лимоны и дальше виды товаров 1 кг 5кг, ящик,  фура',
    quickStartPoint4Note2: 'Идем снова в СКЛАД - ВИДЫ ТОВАРОВ. \n Как мы можем видеть у нас уже сформировались некоторые виды товаров, давайте нажмем на один и посмотрим что там',
    ...
    quickStartPoint5Note8: 'cесли вы заполните все эти поля - тогда на странице Покупок будет указана дополнительная статистика по вашей прибыли расчитанная на основе этих данных',
