...
const mailoutTranslations: Record<LanguageType, Record<string, string>> = {
  ...
  ru: {
    ...
    saveButton: 'Далее необходимо нажать кнопку "СОХРАНИТЬ". Рассылки вида "Разовая" будут запущены автоматически при создании. Другие рассылки необходимо запускать в ручном режиме либо ожидать времени исполнения в автоматическом. Так же существует  возможность авто-удаления сообщений в каналах и чатах после рассылки. В настройках рассылки, рядом с выбором каналов есть поле - кол-во минут. Если установите в нем например 60 минут - час, то этот пост будет удален через час !',
