...
const oneTimeDiscountsTranslations: TranslationType = {
  ...
  ru: {
    ...
    step1: "1. Создать бота через @botfather если не знаеете как сделать это перечитайте весь справочник заново",
    step2: "2. Полученный токен добавляем в поле BOT Token, админ в группе для проверки подписки И ВАЖНО НЕ ЗАБЫТЬ ДОБАВИТЬ ЭТОГО ЖЕ БОТА АДМИНИСТРАТОРОМ В ЧАТ ИЛИ КАНАЛ на который хотим что бы люди обязательно подписывались",
    ...
    oneTimeDiscountsDesc: "Не самый используемый модуль. Он существует по причине того что разовые скидки можно выдавать редактируя карточку клиенты напрямую, данная опция находится внизу настроек каждого клиента при открытии, рекомендуем использовать данный модуль через раздел КЛИЕНТЫ. Если же вы хотите выдать разовую скидку через данный раздел просто заполните все поля используя лупу как переходник для выбора нужных значений и сохраните что бы клиент получил разовую скидку на выбранный тобою товар",
    speedTip: "Вы так же можете изменить баланс напрямую в карточке клиента, для этого рекомендуем ознакомиться с разделом ",
