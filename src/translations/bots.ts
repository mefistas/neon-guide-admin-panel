...
const botsTranslations: BotsTranslations = {
  ru: {
    ...
    botFatherTitle: "Для работы с данным разделом вам необходимо получить хотя бы один токен для установки бота который вам может выдать @BotFather",
    botFatherStep1: "Нажмите старт и пришлите боту команду /newbot",
    botFatherStep2: "Придумайте название которое хотите дать боту например - (КРАКЕН)",
    botFatherStep3: "После этого придумываем юзернейм и тоже отправляем боту",
    botFatherImportant: "ВАЖНО: юзернейм должен быть строго на английском и заканчиваться окончанием bot.",
    botFatherToken: "После этого BotFather пришлет вам длинное сообщение с вашим токеном, этот токен можно скопировать нажав на него, выглядит он примерно так:",
    tokenExample: "8102468:AAG1CBNxxxxxoXjeTpT5CkCdKDN4OgG4xw",
    adminPanelSteps: "Заходим в админ панель раздел БОТЫ/КАНАЛЫ - РАБОЧИЕ БОТЫ",
    addBotSteps: "В верхнем правом углу нажимаем на кнопку ДОБАВИТЬ БОТ и устанавливаем галочку напротив нашего магазина. Вставляем токен который нам дал BotFather в пустое поле и выбираем тип бота который хотите создать",
    ...
    sellerBotSubscription: "В случае если у вас есть каналы и в магазине установлен пункт обязательной подписки на них не забывайте проставлять галочку НЕ ТРЕБУЕТ ПОДПИСКИ если для данного бота она не нужна",
    sellerBotBonus: "Так же устанавливаем бонус для новых клиентов если вы хотите добавить эту фкнкцию",
    sellerBotPrivate: "Бот продавец будет всегда выдаваться покупателям при использовании бота распределителя, если же вы хотите что бы он был скрыт от чужих глаз переведите его в статус ПРИВАТНЫЙ-ПРОДАВЕЦ",
    sellerBotCustomization: "Для более тонкой настройки и кастомизации бота перейдите в раздел",
    customBotLink: "Кастомизация Бота",
    
    // Private Bot
    privateBotTitle: "ПРИВАТНЫЙ БОТ:",
    ...
    privateBotHowToUse: "Как это использовать?",
    ...
    // Distributor Bot
    distributorBotTitle: "БОТ-РАСПРЕДЕЛИТЕЛЬ:",
    ...
    // Payment Bot
    paymentBotTitle: "БОТ ОБРАБОТКИ ПЛАТЕЖЕЙ:",
    ...
    paymentBotWork: "Работа с данным ботом подразумевает что оператор сам будет заходить в него и при готовности клиента купить какой либо товар копировать и отправлять ему нужный кошелек и сумму для пополнения после чего выдаст соответствующий товар руками использовав функцию выдачи товара вручную о том как это сделать читайте в разделе",
    addressesLink: "АДРЕСА",
    
    // Photo Hosting Bots
    photoHostingTitle: "БОТЫ ФОТОХОСТЕРЫ:",
    photoHostingDescription: "Для работы с данным ботом помимо токена вам необходимо придумать пароль который вы будете выдавать вашим сотрудникам для работы с данным ботом, этот пароль сбрасывается раз в несколько дней что бы уменьшить ненужную нагрузку на нашу систему, без этого пароля бот не будет давать возможность загружать фотографии",
    photoHostingUsage: "После успешного ввода пароля просто пришлите боту любое фото и он сделает из него ссылку !",
    
    // FAQ Bots
    faqBotsTitle: "БОТЫ-FAQ",
    faqBotsSetup: "Подключаем бот-ответ/вопрос точно так же как делали ранее, теперь начинается самое интересное",
    faqBotsMenu: "В разделе БОТЫ/КАНАЛЫ снизу есть пункт меню FAQ заходим туда и нажимаем ДОБАВИТЬ FAQ",
    faqBotsConfig: "Указываем наш магазин и бот ответственный за данную задачу и прописываем всю необходимую информацию которая может понадобиться нашим клиентам создавая для каждого раздела свой заголовок",
    ...
    faqBotsHelperLink: "Так же в нашем сервисе предусмотрены боты-помощники, подробнее о их пользе вы можете ознакомиться в разделе",
    helperBotsLink: "БОТЫ-ПОМОЩНИКИ",
  ...
  en: {
    ...
    botFatherTitle: "To work with this section, you need to get at least one token for setting up a bot, which you can get from @BotFather",
    botFatherStep1: "Click start and send the command /newbot to the bot",
    botFatherStep2: "Think of a name you want to give the bot, for example - (KRAKEN)",
    botFatherStep3: "After that, think of a username and also send it to the bot",
    botFatherImportant: "IMPORTANT: The username must be strictly in English and end with the ending bot.",
    botFatherToken: "After that, BotFather will send you a long message with your token, you can copy this token by clicking on it, it looks something like this:",
    tokenExample: "8102468:AAG1CBNxxxxxoXjeTpT5CkCdKDN4OgG4xw",
    adminPanelSteps: "Go to the admin panel section BOTS/CHANNELS - WORKING BOTS",
    addBotSteps: "In the upper right corner, click the ADD BOT button and check the box next to our store. Insert the token that BotFather gave us in the empty field and select the type of bot you want to create",
    ...
    sellerBotSubscription: "If you have channels and the store has a mandatory subscription to them, don't forget to check the NO SUBSCRIPTION REQUIRED box if this bot doesn't need it",
    sellerBotBonus: "Also set a bonus for new clients if you want to add this function",
    sellerBotPrivate: "The seller bot will always be given to buyers when using the distributor bot, but if you want it to be hidden from prying eyes, convert it to PRIVATE-SELLER status",
    sellerBotCustomization: "For more fine-tuning and customization of the bot, go to the section",
    customBotLink: "Bot Customization",
    
    // Private Bot
    privateBotTitle: "PRIVATE BOT:",
    ...
    privateBotHowToUse: "How to use it?",
    ...
    // Distributor Bot
    distributorBotTitle: "DISTRIBUTOR BOT:",
    ...
    // Payment Bot
    paymentBotTitle: "PAYMENT PROCESSING BOT:",
    ...
    paymentBotWork: "Working with this bot implies that the operator himself will enter it and, when the client is ready to buy any product, copy and send him the necessary wallet and the amount for replenishment, after which he will issue the corresponding product manually using the manual product issuance function, for more information on how to do this, see the section",
    addressesLink: "ADDRESSES",
    
    // Photo Hosting Bots
    photoHostingTitle: "PHOTO HOSTING BOTS:",
    photoHostingDescription: "To work with this bot, in addition to the token, you need to come up with a password that you will give to your employees to work with this bot, this password is reset once every few days to reduce unnecessary load on our system, without this password, the bot will not allow you to upload photos",
    photoHostingUsage: "After successfully entering the password, simply send any photo to the bot and it will turn it into a link!",
    
    // FAQ Bots
    faqBotsTitle: "FAQ BOTS",
    faqBotsSetup: "We connect the question/answer bot in the same way as we did earlier, now the most interesting part begins",
    faqBotsMenu: "In the BOTS/CHANNELS section at the bottom there is a FAQ menu item, we go there and click ADD FAQ",
    faqBotsConfig: "We specify our store and the bot responsible for this task and write all the necessary information that our clients may need, creating our own heading for each section",
    ...
    faqBotsHelperLink: "Also in our service there are helper bots, you can learn more about their benefits in the section",
    helperBotsLink: "HELPER BOTS",
